...
| `aliases` | `AliasEntry[]` | Variant-to-canonical mappings. A slot filled through an alias reports the **canonical** value, never the variant that was spoken. Variants carry the same constraints and the same warnings as `values`. |
