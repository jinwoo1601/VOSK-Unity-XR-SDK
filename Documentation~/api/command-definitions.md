...
| `Aliases` | `Dictionary<string, string>` | Maps variant words to canonical values. A slot filled through an alias reports the **canonical** value, never the variant the speaker said. Keys carry the same constraints and the same warnings as `Values`. **`null`** when no aliases were supplied — the constructor stores `null` rather than an empty dictionary, so code that iterates this must null-check first. |
