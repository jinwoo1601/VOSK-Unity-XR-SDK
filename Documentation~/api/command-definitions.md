...
| `AllowPartialMatch` | `bool` | When true, a match with unfilled required slots enters pending state instead of being rejected, allowing follow-up speech to fill the gaps — one slot per utterance if that is how they arrive. It also lets a [confirm phrase fire the command with its arguments still absent](../command-recognition.md#the-two-ways-an-incomplete-command-still-fires), so its handler must tolerate that. Default: `false`. |
