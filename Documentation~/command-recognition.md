...
**Until #65 that decided it.** The bare pattern scored a flat 1.0, won selection, and the slot value the speaker *did* say was discarded with nothing to signal it -- "decelerate hard burn" executed a default-level decelerate. No threshold tuning reached it, since nothing normalised to 1.0 can be out-scored. [Coverage](#coverage) closes that hazard: the bare pattern is now charged for the "hard burn" it leaves unexplained, scores `1/(1+2)` = 0.33, and loses to the 0.67. The command fires **with** its argument.

**The warning stands, and the swap is still worth making** -- what changed is the cost of ignoring it. `0.67` clears the default `minScore` by `0.07`, so anything else going wrong in the same utterance puts it back under the gate; and if you set `coverageWeight` to `0`, the old selection behaviour comes back with it. With the literal optional, an omitted optional drops out of both sides of the ratio, so the slot-filled pattern scores 1.0 whether or not the word was spoken and wins outright. Both phrasings then extract the slot, and a bare "decelerate" still matches the bare pattern.
...
Three things are never charged:
...
- **Trailing tokens that could begin another match.** Counting stops at the first token some active pattern could start on, which is what keeps multi-command utterances intact: "cease fire launch missiles target hotel one" scores `cease_fire` at a full `2 / 2`, not `2 / 7`.
