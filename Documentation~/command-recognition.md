...
**Several missing slots take several utterances if they have to.** Follow-up speech fills the unfilled slots in pattern order and stops at the first one it cannot find, so an utterance that answers only part of what is missing leaves the command *still pending* — with what it just filled kept, and `OnCommandPending` fired again carrying the updated command. It fires only once no required slot is left. A prompt driven off `OnCommandPending` therefore sees each fill as it lands and can name what is still outstanding. Nothing is lost by answering one slot at a time, but the whole exchange runs against the single `pendingTimeout` window that started when the command first entered pending — filling a slot does not extend it.
