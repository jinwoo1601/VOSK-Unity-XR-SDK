...
- **Missing a required slot** (the pattern's literals are all in, but an argument has not been spoken yet) -> keeps waiting, even where the arithmetic leaves the partial match above `minScore`. An unspoken slot consumes no words, so such a buffer otherwise looks complete right up to the moment the missing words arrive.
