...
| *(empty, `accepted: true`)* | Follow-up speech filled a pending command's missing slot, and no required slot is left — the command fired. When the utterance filled some but not all of what was still missing, the same entry instead carries `accepted: false` and `still pending (partial: unfilled [...])`: the fill was kept, the command did not fire, and the pending is still live. |
