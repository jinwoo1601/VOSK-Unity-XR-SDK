...
- Documented, at every point of use, that a `NumberSequence` slot value is the **words as spoken** — `cmd.GetSlot("heading")` returns `"two seven zero"`, never `"270"`. The contract lived only in `api/number-parser.md`, which nothing routed a `GetSlot` caller to, so the obvious `int.TryParse(cmd.GetSlot(...))` was the natural thing to write; it fails on every utterance and yields `0` without throwing, which presents as numeric commands that simply never fire while number-free ones work. Worse, three places actively taught the wrong shape, writing the *converted* value as if it were the slot value (`// "heading two seven zero" -> heading="270"` in the command-recognition guide, the same in `api/command-definitions.md`, and `heading = 270` in the sample's "Try saying" table); all three are corrected. `VoxrCommand.GetSlot`/`HasSlot`, `VoxrSlotMatch.Value`, and both `VoxrSlotType` members now carry XML doc comments, so the contract reaches the IDE tooltip at the call site, and `api/data-types.md`, `api/command-definitions.md`, the command-recognition guide, and the sample READMEs all state it and cross-link `api/number-parser.md`. The guide gains a canonical conversion snippet — `ParseDigitSequence` first, `ParseCardinal` as fallback, both in `try`/`catch` since they throw `FormatException` rather than returning a sentinel — alongside the warning that the order is load-bearing because the two read the same words differently (`"two seven zero"` is `270` on the digit path but `9` on the cardinal one, and `"two seventy"`, which only the cardinal path accepts, comes back `72` rather than the `270` a speaker means by it), so one convention per slot beats leaning on the fallback to guess intent. The `CommandRecognition` sample now handles its own `set_heading` intent through that exact helper instead of leaving the one slot type that needs conversion undemonstrated, so copy-paste consumers inherit the correct pattern. Documentation and sample only — no runtime behaviour changes. ([#46](https://github.com/jinwoo1601/VoXR-Speech-Recognition/issues/46))
